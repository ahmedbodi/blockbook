...
// pesoParser handle
type pesoParser struct {
	*btc.BitcoinParser
	baseparser                         *bchain.BaseParser
	BitcoinOutputScriptToAddressesFunc btc.OutputScriptToAddressesFunc
}

// NewpesoParser returns new pesoParser instance
func NewpesoParser(params *chaincfg.Params, c *btc.Configuration) *pesoParser {
	p := &pesoParser{
...
// GetChainParams contains network parameters for the main peso network
...
func (p *pesoParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
	...
	if h.Version > 3 {
		// Skip past AccumulatorCheckpoint which was added in peso block version 4
...
func (p *pesoParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *pesoParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *pesoParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
...
func (p *pesoParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
	...
	for i, in := range t.TxIn {
		...
		txid := in.PreviousOutPoint.peso.String()
...
func (p *pesoParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *pesoParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *pesoParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
	if len(tx.Vin) > input {
...
func (p *pesoParser) GetValueSatForUnknownInput(tx *bchain.Tx, input int) *big.Int {
	if len(tx.Vin) > input {
		scriptHex := tx.Vin[input].ScriptSig.Hex
		if scriptHex != "" {
			script, _ := hex.DecodeString(scriptHex)
...
func (p *pesoParser) GetValueSatFromZerocoinSpend(signatureScript []byte) (*big.Int, error) {
