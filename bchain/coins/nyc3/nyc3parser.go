...
// magic numbers
const (
	MainnetMagic wire.BitcoinNet = 0xf353e35f
...
func NewNyc3Parser(params *chaincfg.Params, c *btc.Configuration) *Nyc3Parser {
	return &Nyc3Parser{BitcoinParser: btc.NewBitcoinParser(params, c)}
}
...
func (p *Nyc3Parser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
