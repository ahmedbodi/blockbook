...
func init() {
	// hashash mainnet Address encoding magics
	...
	// hashash testnet Address encoding magics
...
// NewhashParser returns new hashParser instance
func NewhashParser(params *chaincfg.Params, c *btc.Configuration) *hashParser {
	p := &hashParser{
...
// GetChainParams contains network parameters for the main hash network
...
func (p *hashParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
	...
	if h.Version > 3 {
		// Skip past AccumulatorCheckpoint which was added in hash block version 4
...
func (p *hashParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *hashParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *hashParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
...
func (p *hashParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *hashParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *hashParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *hashParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
	if len(tx.Vin) > input {
...
func (p *hashParser) GetValueSatForUnknownInput(tx *bchain.Tx, input int) *big.Int {
	if len(tx.Vin) > input {
		scriptHex := tx.Vin[input].ScriptSig.Hex
		if scriptHex != "" {
			script, _ := hex.DecodeString(scriptHex)
...
func (p *hashParser) GetValueSatFromZerocoinSpend(signatureScript []byte) (*big.Int, error) {
