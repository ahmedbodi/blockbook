...
// qbcParser handle
type qbcParser struct {
	*btc.BitcoinParser
}

// NewqbcParser returns new qbcParser instance
func NewqbcParser(params *chaincfg.Params, c *btc.Configuration) *qbcParser {
	return &qbcParser{BitcoinParser: btc.NewBitcoinParser(params, c)}
}

// GetChainParams contains network parameters for the main qbc network
...
func (p *qbcParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
